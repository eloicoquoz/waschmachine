...
const INIT_NOTIFS = [
  {id:1,read:false,type:"reminder_response",text:"N'oublie pas de répondre à un tournoi !",tid:null,time:"Il y a 1h"},
];

const NOTIF_TYPES = {
  reminder_response:  {icon:"⏰"},
  new_tournament:     {icon:"🏐"},
  new_feed:           {icon:"📸"},
  new_chat:           {icon:"💬"},
  tournament_reminder:{icon:"📅"},
  result_added:       {icon:"🏆"},
};

/* ════ HELPERS ════ */
const isPast    = (t) => new Date(t.rawDate) < new Date();
const myAnswer  = (t) => t.yesResponses&&t.yesResponses.includes(ME) ? "yes"
                        : t.noResponses&&t.noResponses.includes(ME)  ? "no" : null;
...
function Shell({children,notif,onNotif,tab,onTabChange,unread,memberCount}) {
  ...
  return (
    <div style={{background:"#0A1220",minHeight:"100vh",display:"flex",justifyContent:"center",padding:"28px 0 40px",fontFamily:"'Inter','Segoe UI',sans-serif"}}>
      <div style={{width:390,background:C.bg,borderRadius:44,overflow:"hidden",position:"relative",display:"flex",flexDirection:"column",boxShadow:`0 60px 120px #000d,0 0 0 1px ${C.border}`,minHeight:844}}>
        <div style={{background:C.navy,padding:"14px 28px 8px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{color:C.text,fontSize:13,fontWeight:600}}>9:41</span>
          <div style={{background:"#000",width:120,height:28,borderRadius:20}}/>
          <span style={{color:C.text,fontSize:13}}>●●●</span>
        </div>
        <div style={{background:C.navy,padding:"10px 16px 14px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
          <WaschLogo size={46}/>
          <div>
            <div style={{color:C.gold,fontWeight:800,fontSize:15,letterSpacing:2}}>WASCHMACHINE</div>
            <div style={{color:C.muted,fontSize:10,letterSpacing:0.5}}>VOLLEYBALL CLUB · {memberCount||"…"} membres</div>
          </div>
        ...
        {onTabChange&&(
          <div style={{position:"absolute",bottom:0,left:0,right:0,background:C.navy,borderTop:`1px solid ${C.border}`,display:"flex",paddingBottom:20,paddingTop:8}}>
            {tabs.map(t=>(
...
export default function App() {
  ...
  /* Handlers */
  const handleResponse=(id,ans)=>{
    respondToTournament(String(id),ME,ans);
    toast2(ans==="yes"?"Inscrit !":"Réponse enregistrée",ans==="yes"?C.cyan:C.muted);
  };
  const handleRemove=(id)=>{removeResponse(String(id),ME);toast2("Désinscrit",C.coral);};
  const handleValidate=(id)=>{
    const t=ts.find(t=>t.id===id);
    validateTeam(String(id),!t.teamValidated);
    toast2(!t.teamValidated?"Équipe validée ✓":"Validation annulée",!t.teamValidated?C.cyan:C.muted);
  };
  ...
  const handleCreate=async()=>{
    ...
    await createTournament({
      name:form.name.trim(),date:dl,rawDate:form.rawDate,lieu:form.lieu.trim(),
      playType:form.playType,surface:form.playType==="Indoor"?null:form.surface,
      playerFormat:form.playerFormat,deadline:"—",
      minPlayers:parseInt(form.minPlayers)||2,maxPlayers:parseInt(form.maxPlayers)||8,
      proposedBy:ME,description:form.description.trim(),flyerUrl:null,
    });
  ...
  const handleAddPost=async(post)=>{
    let mediaUrl=null;
    if(post.mediaSrc){
      const res=await fetch(post.mediaSrc);const blob=await res.blob();
      const file=new File([blob],"media."+(blob.type.split("/")[1]||"jpg"),{type:blob.type});
      mediaUrl=await uploadMedia(file,ME);
    }
  ...
  const notifClick=(n)=>{
    setNtfs(p=>p.map(x=>x.id===n.id?{...x,read:true}:x));
    if(n.tid){setSelId(n.tid);setTab("tournois");}setView("main");
  };
  ...
  const handleNotifications=()=>{
    ...
    if(Notification.permission==="granted"){
      new Notification("Waschmachine 🏐",{body:"Les notifications sont déjà activées !"});
      toast2("Notifications actives ✓",C.cyan);return;
    }
    if(Notification.permission==="denied"){
      alert("Notifications bloquées.\n\nDans Edge/Chrome : cliquer sur 🔒 dans la barre d'adresse → Autorisations du site → Notifications → Autoriser");return;
    }
    Notification.requestPermission().then(perm=>{
      if(perm==="granted"){
        toast2("Notifications activées ✓",C.cyan);
        new Notification("Waschmachine 🏐",{body:"Tu recevras les alertes de l'asso !"});
      }else toast2("Permission refusée",C.coral);
    });
  };

  const handleInstall=()=>{
    if(window._installPrompt){window._installPrompt.prompt();}
    else toast2("iOS : Safari → Partager → Ajouter à l'écran d'accueil",C.gold);
  };

  /* Shell shorthand */
  const shell=(c)=><Shell notif={unreadNtfs} onNotif={()=>setView("notifs")} memberCount={members.length}>{c}</Shell>;
  ...
  return (
    <Shell notif={unreadNtfs} onNotif={()=>setView("notifs")} tab={tab} onTabChange={setTab} unread={unread} memberCount={members.length}>
      ...
      {toast&&<div style={{position:"absolute",bottom:100,left:"50%",transform:"translateX(-50%)",background:toast.color+"22",border:`1px solid ${toast.color}`,color:toast.color,padding:"10px 22px",borderRadius:24,fontSize:13,fontWeight:700,whiteSpace:"nowrap",zIndex:200,pointerEvents:"none"}}>{toast.msg}</div>}
    </Shell>
  );
}
...
function HomeTab({myTs,onSel,onChat,unread,onCal}) {
  ...
  return (
    <div style={{overflowY:"auto",flex:1,padding:"20px 16px",paddingBottom:84}}>
            ...
            <div key={t.id} style={{background:t.teamValidated?C.cyan+"12":C.card,borderRadius:16,padding:16,border:`2px solid ${C.cyan}55`,marginBottom:12,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{paddingLeft:8}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button onClick={()=>onChat(t.id)} style={{flex:1,padding:"8px 0",borderRadius:10,border:`1px solid ${hu?C.coral:C.border}`,background:hu?C.coral+"18":"none",color:hu?C.coral:C.gold,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
                    💬 Chat{hu?" 🔴":""}
                  </button>
                </div>
              </div>
...
function FeedTab({feed,onLike,onAdd,onDelete,currentUser}) {
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:84}}>
      <div style={{padding:"16px 16px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${C.border}`}}>
        ...
        return (
          <div key={p.id} style={{borderBottom:`1px solid ${C.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px"}}>
              ...
              <div>
                <div style={{color:C.text,fontWeight:700,fontSize:13}}>{p.authorName||p.author}</div>
                <div style={{color:C.muted,fontSize:11}}>{p.time||""}</div>
              </div>
              <div style={{marginLeft:"auto",position:"relative"}}>
                ...
                {menuId===p.id&&(
                  <div style={{position:"absolute",right:0,top:30,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",zIndex:10,minWidth:140,boxShadow:"0 8px 24px #0008"}}>
            ...
            {p.mediaUrl&&p.mediaType==="image"&&<img src={p.mediaUrl} alt="" style={{width:"100%",height:280,objectFit:"cover",display:"block"}}/>}
            {p.mediaUrl&&p.mediaType==="video"&&<video src={p.mediaUrl} controls playsInline style={{width:"100%",maxHeight:340,objectFit:"cover",display:"block",background:"#000"}}/>}
            {(!p.mediaUrl||p.mediaType==="gradient")&&<div style={{height:240,background:p.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:80}}>{p.emoji}</div>}
            <div style={{padding:"12px 16px"}}>
              <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:8}}>
                <button onClick={()=>onLike(p.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:5,padding:0}}>
...
function AddPostView({onPost,onBack}) {
  ...
  const handleFile=e=>{
    const fi=e.target.files&&e.target.files[0];if(!fi)return;
    const isVid=fi.type.startsWith("video/");
    const r=new FileReader();r.onload=ev=>{setMediaSrc(ev.target.result);setMType(isVid?"video":"image");};r.readAsDataURL(fi);
  };
  const handlePost=()=>{
    if(!caption.trim()&&!mediaSrc)return;
    onPost({author:ME,caption:caption.trim(),mediaSrc,mediaType:mediaSrc?mediaType:"gradient",grad:FEED_GRADS[gradIdx],emoji});
  };
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:20}}>
        <div style={{position:"relative",background:"#000",cursor:"pointer"}} onClick={()=>{if(!hasMedia)fileRef.current&&fileRef.current.click();}}>
          {!hasMedia&&<div style={{height:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:FEED_GRADS[gradIdx],gap:10}}>
            ...
            <div style={{background:"#0008",borderRadius:24,padding:"10px 20px",display:"flex",alignItems:"center",gap:8}} onClick={e=>{e.stopPropagation();fileRef.current&&fileRef.current.click();}}>
              ...
              <span style={{color:"#fff",fontWeight:700,fontSize:14}}>Ajouter une photo ou vidéo</span>
            </div>
          </div>}
          {hasMedia&&mediaType==="image"&&<img src={mediaSrc} alt="" style={{width:"100%",maxHeight:400,objectFit:"cover",display:"block"}}/>}
          {hasMedia&&mediaType==="video"&&<video src={mediaSrc} controls playsInline style={{width:"100%",maxHeight:400,display:"block",background:"#000"}}/>}
        ...
        <div style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:`1px solid ${C.border}`}}>
          ...
          <textarea value={caption} onChange={e=>setCaption(e.target.value)} placeholder="Écris une légende..." style={{flex:1,background:"none",border:"none",color:C.text,fontSize:15,resize:"none",outline:"none",fontFamily:"inherit",minHeight:80,lineHeight:1.5}}/>
        </div>
        {!hasMedia&&<div style={{padding:"14px 16px"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {FEED_GRADS.map((g,i)=><div key={i} onClick={()=>setGradIdx(i)} style={{width:40,height:40,borderRadius:10,background:g,cursor:"pointer",border:`3px solid ${gradIdx===i?C.gold:"transparent"}`,flexShrink:0}}/>)}
          </div>
          <div style={{color:C.muted,fontSize:12,marginBottom:8}}>Emoji</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {FEED_EMOJIS.map(e=><button key={e} onClick={()=>setEmoji(e)} style={{width:40,height:40,borderRadius:10,background:emoji===e?C.gold+"22":C.card,border:`2px solid ${emoji===e?C.gold:C.border}`,fontSize:20,cursor:"pointer"}}>{e}</button>)}
          </div>
        </div>}
        <div style={{padding:"16px 16px 0"}}>
          <button onClick={handlePost} style={{width:"100%",padding:16,borderRadius:14,border:"none",background:`linear-gradient(135deg,${C.gold},${C.gold}CC)`,color:C.navy,fontWeight:800,fontSize:16,cursor:"pointer",fontFamily:"inherit",opacity:caption.trim()||mediaSrc?1:0.5}}>Publier</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={()=>onSel(t.id)} style={{background:bg,borderRadius:16,padding:16,border:`${bw} solid ${bc}`,cursor:"pointer",position:"relative",overflow:"hidden",opacity:past?0.9:1}}>
      ...
      <div style={{paddingLeft:!past&&ans!==null?8:0}}>
        ...
        {!past&&<div>
          ...
          <div style={{background:C.dimmed,borderRadius:3,height:3,overflow:"hidden"}}>
            <div style={{background:`linear-gradient(90deg,${C.green},${C.cyan})`,width:`${pct}%`,height:"100%",borderRadius:3}}/>
          </div>
        </div>}
...
function TournoisTab({ts,onSel,onAdd}) {
  const upcoming=ts.filter(t=>!isPast(t));
  const past=ts.filter(t=>isPast(t));
  const [showPast,setShowPast]=useState(true);
  return (
    <div style={{overflowY:"auto",flex:1,paddingBottom:84}}>
      <div style={{padding:"20px 16px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>{upcoming.map(t=><TCard key={t.id} t={t} onSel={onSel}/>)}</div>
...
function TModal({t,onClose,onResponse,onRemove,onValidate,onAddResult,onChat}) {
  ...
  return (
    <div onClick={onClose} style={{position:"absolute",inset:0,background:"#000c",display:"flex",flexDirection:"column",justifyContent:"flex-end",zIndex:50}}>
      <div onClick={e=>e.stopPropagation()} style={{background:modalBg,borderRadius:"28px 28px 0 0",padding:24,borderTop:`2px solid ${topColor}44`,maxHeight:"92%",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
          <div>
            <div style={{color:C.text,fontWeight:800,fontSize:20}}>{t.name}</div>
            <div style={{color:C.muted,fontSize:12,marginTop:4}}>Proposé par {t.proposedBy}{isAdmin?" (toi)":""}</div>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          {[{icon:"📅",l:"Date",v:t.date},{icon:"📍",l:"Lieu",v:t.lieu},{icon:"🏐",l:"Format",v:`${t.playerFormat} ${fmtType(t)}`},{icon:"⏰",l:"Deadline",v:t.deadline}].map(i=>(
            <div key={i.l} style={{background:C.card,borderRadius:10,padding:"10px 12px"}}>
              <div style={{color:C.muted,fontSize:11,marginBottom:2}}>{i.icon} {i.l}</div>
              <div style={{color:C.text,fontSize:12,fontWeight:600}}>{i.v}</div>
            </div>
          ))}
        </div>
        <div style={{marginBottom:14}}>
          <div style={{color:cap.color,fontWeight:700,fontSize:13,marginBottom:8}}>{cap.text}</div>
          {(t.yesResponses||[]).length>0&&<>
            <div style={{color:C.green,fontSize:11,fontWeight:700,marginBottom:5}}>OUI — {(t.yesResponses||[]).length}</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
              {(t.yesResponses||[]).map(n=><div key={n} style={{background:n===ME?C.green+"22":C.card,border:`1px solid ${n===ME?C.green+"88":C.border}`,color:n===ME?C.green:C.text,fontSize:11,fontWeight:600,padding:"4px 10px",borderRadius:20}}>{n===ME?"Toi ✓":n}</div>)}
            </div>
          </>}
          {(t.noResponses||[]).length>0&&<>
            <div style={{color:C.coral,fontSize:11,fontWeight:700,marginBottom:5}}>NON — {(t.noResponses||[]).length}</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {(t.noResponses||[]).map(n=><div key={n} style={{background:n===ME?C.coral+"22":C.card,border:`1px solid ${n===ME?C.coral+"88":C.border}`,color:n===ME?C.coral:C.muted,fontSize:11,padding:"4px 10px",borderRadius:20}}>{n===ME?"Toi ✕":n}</div>)}
            </div>
          </>}
        </div>
          ...
          {!addingRes
            ...
            :<div style={{background:C.card,borderRadius:14,padding:16,border:`1px solid ${C.gold}44`}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                <div>
                  <div style={{color:C.muted,fontSize:12,marginBottom:6}}>Classement</div>
                  <select value={rForm.placement} onChange={e=>setRForm(p=>({...p,placement:parseInt(e.target.value)}))} style={IS({cursor:"pointer"})}>
                    {[1,2,3,4,5,6,7,8].map(n=><option key={n} value={n} style={{background:C.card}}>{n===1?"🥇 1re":n===2?"🥈 2e":n===3?"🥉 3e":n+"e"} place</option>)}
                  </select>
                </div>
                <div>
                  <div style={{color:C.muted,fontSize:12,marginBottom:6}}>Total équipes</div>
                  <input type="number" min={2} max={64} value={rForm.totalTeams} onChange={e=>setRForm(p=>({...p,totalTeams:parseInt(e.target.value)}))} style={IS()}/>
                </div>
              </div>
              <div style={{marginBottom:12}}>
                <div style={{color:C.muted,fontSize:12,marginBottom:6}}>Commentaire</div>
                <textarea value={rForm.notes} onChange={e=>setRForm(p=>({...p,notes:e.target.value}))} placeholder="Ambiance, résultats..." style={IS({resize:"none",height:70})}/>
              </div>
        ...
        {isAdmin&&!past&&<div style={{background:C.card,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${C.gold}33`}}>
          ...
          <button onClick={()=>onValidate(t.id)} style={{width:"100%",padding:12,borderRadius:10,border:`1px solid ${t.teamValidated?C.cyan:C.border}`,background:t.teamValidated?C.cyan+"22":"none",color:t.teamValidated?C.cyan:C.muted,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
            {t.teamValidated?"✓ Équipe inscrite — cliquer pour annuler":"Marquer l'équipe comme inscrite ✓"}
          </button>
        </div>}
        {!past&&<div>
...
function ChatsTab({myTs,unread,onOpen}) {
  return (
    <div style={{overflowY:"auto",flex:1,paddingBottom:84}}>
      <div style={{padding:"20px 16px"}}>
        <div style={{color:C.text,fontSize:22,fontWeight:800,marginBottom:4}}>Chats</div>
        <div style={{color:C.muted,fontSize:13,marginBottom:20}}>Un chat par tournoi où tu participes</div>
        {myTs.length===0
...
function ChatView({t,onSend,onBack}) {
  ...
  useEffect(()=>{
    if(!t||!t.id)return;
    return subscribeChat(String(t.id),data=>{setMsgs(data);setLoading(false);});
  },[t&&t.id]);
  const send=()=>{onSend(inp);setInp("");};
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",paddingBottom:84}}>
      <div style={{background:C.surface,padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:C.gold,fontSize:22,cursor:"pointer",padding:0}}>←</button>
        <div>
          <div style={{color:C.text,fontWeight:700,fontSize:15}}>{t.name}</div>
          <div style={{color:C.muted,fontSize:11}}>{(t.yesResponses||[]).length} participants</div>
        </div>
      </div>
      ...
      <div style={{padding:"8px 16px 16px",borderTop:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"center"}}>
      ...
      <div style={{padding:"0 16px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:10,marginBottom:form.playType==="Outdoor"?12:0}}>
            {["Indoor","Outdoor"].map(tp=>(
              <button key={tp} onClick={()=>f("playType",tp)} style={{flex:1,padding:"11px 0",borderRadius:12,border:`2px solid ${form.playType===tp?C.gold:C.border}`,background:form.playType===tp?C.gold+"15":"none",color:form.playType===tp?C.gold:C.muted,fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>
                {tp==="Indoor"?"🏟 Indoor":"🌤 Outdoor"}
              </button>
            ))}
          </div>
          {form.playType==="Outdoor"&&<div style={{display:"flex",gap:8}}>
            {surfaces.map(s=>(
              <button key={s} onClick={()=>f("surface",s)} style={{flex:1,padding:"9px 0",borderRadius:10,border:`2px solid ${form.surface===s?C.cyan:C.border}`,background:form.surface===s?C.cyan+"18":"none",color:form.surface===s?C.cyan:C.muted,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
                {s==="Beach"?"🏖 Beach":s==="Gazon"?"🌿 Gazon":"💧 Water"}
              </button>
            ))}
          </div>}
        </div>
        ...
        <div onClick={()=>f("iParticipate",!form.iParticipate)} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:C.card,borderRadius:12,border:`1px solid ${C.border}`,cursor:"pointer"}}>
          <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${form.iParticipate?C.gold:C.border}`,background:form.iParticipate?C.gold:"none",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            {form.iParticipate&&<span style={{color:C.navy,fontSize:14,fontWeight:800}}>✓</span>}
          </div>
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:20}}>
      ...
      <div style={{padding:"0 20px",display:"flex",flexDirection:"column",gap:16}}>
        {[
          ...
          {icon:"💰",title:"Frais",text:"Les frais d'inscription aux tournois sont couverts par la cotisation annuelle. Pas de frais supplémentaires à prévoir par tournoi."},
...
function ProfilTab({ts,picSrc,setPic,members,onNotif,onInstall,onCharte}) {
  ...
  // Most active from real Firebase members
  const mostActive=members.length>0
    ? members.map(m=>({name:m.displayName||m.email||"?",n:ts.filter(t=>(t.yesResponses||[]).includes(m.displayName||"")).length})).sort((a,b)=>b.n-a.n)[0]
    : {name:"—",n:0};
    ...
    <div style={{overflowY:"auto",flex:1,paddingBottom:84}}>
      ...
      {palmares.length>0&&(
        <div style={{padding:"0 16px",marginBottom:20}}>
          ...
          {bestP&&<div style={{background:C.card,borderRadius:14,padding:14,marginBottom:10,border:`1px solid ${plcColor(bestP)}44`,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              <div style={{color:plcColor(bestP),fontWeight:800,fontSize:15}}>Meilleur : {plcLabel(bestP)}</div>
              <div style={{color:C.muted,fontSize:12}}>{palmares.find(t=>t.result.placement===bestP)?.name}</div>
            </div>
          </div>}
          {palmares.map(t=>(
      ...
      <div style={{padding:"0 16px",marginBottom:20}}>
        ...
        <div style={{background:C.card,borderRadius:16,padding:16,border:`1px solid ${C.border}`}}>
          {[
            {l:"Membres inscrits",v:members.length,icon:"👥"},
            {l:"Tournois à venir",v:ts.filter(t=>!isPast(t)).length,icon:"🏆"},
            {l:"Tournois joués",v:ts.filter(t=>isPast(t)).length,icon:"✅"},
            {l:"Total inscriptions",v:totalYes,icon:"📊"},
            {l:"Membre le + actif",v:mostActive.name,icon:"⭐"},
          ].map(s=>(
      ...
      <div style={{padding:"0 16px"}}>
        ...
        {[
          {icon:"🔔",l:"Notifications",sub:"Activer les rappels et alertes",fn:onNotif},
          {icon:"📱",l:"Installer l'app",sub:"Ajouter à l'écran d'accueil",fn:onInstall},
          {icon:"📋",l:"Charte Waschmachine",sub:"Règles de l'association",fn:onCharte},
        ].map(it=>(
...
/* ════ AUTH ════ */
const PW={background:"#0A1220",minHeight:"100vh",display:"flex",justifyContent:"center",padding:"28px 0 40px",fontFamily:"'Inter','Segoe UI',sans-serif"};
const PB={width:390,background:C.bg,borderRadius:44,overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:`0 60px 120px #000d,0 0 0 1px ${C.border}`,minHeight:844};
function SBar(){return <div style={{background:C.navy,padding:"14px 28px 8px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}><span style={{color:C.text,fontSize:13,fontWeight:600}}>9:41</span><div style={{background:"#000",width:120,height:28,borderRadius:20}}/><span style={{color:C.text,fontSize:13}}>●●●</span></div>;}
  ...
  return (
    <div style={PW}><div style={PB}>
      ...
      <div style={{padding:"16px 28px 44px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{color:C.muted,fontSize:11,textAlign:"center",marginTop:6,lineHeight:1.5}}>En continuant, tu acceptes les règles de l'asso Waschmachine.</div>
      </div>
    </div></div>
  );
}
...
function EmailAuthScreen({onBack,onSuccess}) {
  ...
  return (
    <div style={PW}><div style={PB}>
      <SBar/>
      <div style={{padding:"16px 20px 14px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${C.border}`,background:C.surface,flexShrink:0}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:C.gold,fontSize:22,cursor:"pointer",padding:0}}>←</button>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"24px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
        <div style={{display:"flex",background:C.card,borderRadius:12,padding:4,gap:4}}>
          {["signup","signin"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setError("");}} style={{flex:1,padding:"10px 0",borderRadius:10,border:"none",background:mode===m?C.gold:"none",color:mode===m?C.navy:C.muted,fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>
          ...
          <div style={{position:"relative",marginBottom:24,cursor:"pointer"}} onClick={()=>picRef.current&&picRef.current.click()}>
            <div style={{width:90,height:90,borderRadius:"50%",border:`3px solid ${C.gold}`,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
              {avatar?<img src={avatar} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                :<div style={{width:"100%",height:"100%",background:`linear-gradient(135deg,${C.gold},${C.cyan})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,fontWeight:800,color:C.navy}}>{dName[0]||"?"}</div>}
            </div>
